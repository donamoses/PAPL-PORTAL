...
export default class TaskListWebPart extends BaseClientSideWebPart<ITaskListWebPartProps> {
  ...
  private loadLists(): Promise<IPropertyPaneDropdownOption[]> {
    let url = "https://mrbutlers.sharepoint.com/sites/TaskManagement";
    //let url = this.properties.siteUrl;
    //http://<domain>/<site url>/_api/lists/getbytitle('FAQ')/items?$select=Title,Answer
    return this.context.spHttpClient.get(url + `/_api/web/lists`, SPHttpClient.configurations.v1) // sending the request to SharePoint REST API
   ...
   private loadItems(): Promise<IPropertyPaneDropdownOption[]> {
    ...
    let url = "https://mrbutlers.sharepoint.com/sites/TaskManagement";
    //let url = this.properties.siteUrl;
    return this.context.spHttpClient.get(url + '/_api/web/lists(guid\'' + listId + '\')/views', SPHttpClient.configurations.v1) // requesting views from SharePoint REST API
    .then((response: SPHttpClientResponse) => { // httpClient.get method returns a response object where json method creates a Promise of getting result
    return response.json();
    })
