...
export default class TaskListViewModel {
  ...
  constructor(bindings: ITaskListBindingContext) {
    ...
    var url = "https://mrbutlers.sharepoint.com/sites/TaskManagement";//bindings.context.pageContext.web.absoluteUrl;
    this.BaseUrl(url);
  ...
  public DisplayListView(web: Web, listId: string, viewId: string, userColl: Array<ISiteUser>) {
    ...
    web.lists.getById(listId).views.getById(viewId).fields.get().then(val => {
      ...
      web.lists.getById(listId).views.getById(viewId).get().then(xx => {
        ...
        _viewXML = _viewXML.replace('<RowLimit Paged="TRUE">30</RowLimit>', '<RowLimit Paged="False">3</RowLimit>');
            ...
            let itmValue: Array<IDict> = new Array<IDict>();
              ...
              if (x.Title == "AssignedTo") {
                ...
                if (result && result.length !== 0) {
                  itmValue.push({ key: x.Title, value: result[0]['UserTitle'], url: null,background:false });
                }
                else {
                  itmValue.push({ key: x.Title, value: '', url: null,background:false });
                }
              }
              else {
                if (x.Title == "Predecessors") {
                  itmValue.push({ key: x.Title, value: element['PredecessorsId'], url: null,background:false });
                }
                else {
                  if (x.Title == "WorkflowLink") {
                    if(element['WorkflowLink']) {
                      itmValue.push({ key: x.Title, value: element['WorkflowLink']['Description'], url: element['WorkflowLink']['Url'],background:false });
                    }else {
                      itmValue.push({ key: x.Title, value: "", url: "",background:false });
                    }
                  }
                  else {
                    if (x.Title == "LinkTitle") {
                      ...
                      itmValue.push({ key: x.Title, value: element['Title'], url: actionUrl,background:false });
                    }
                    else {
                      if (x.Title == "DueDate") {
                        var dt;
                        ...
                        // } else {
                          ...
                          {
                            itmValue.push({ key: x.Title, value: dt.toDateString(), url: null,background:true });
                          }
                          else
                          {
                            itmValue.push({ key: x.Title, value: dt.toDateString(), url: null,background:false });
                          }
                      //  }
                        ...
                        dt = new Date(element['MM_x0020_Due_x0020_Date']);
                        if( new Date(element['MM_x0020_Due_x0020_Date']) <new Date())
                          {
                            itmValue.push({ key: "Due Date", value: dt.toDateString(), url: null,background:true });
                          }
                          else
                          {
                            itmValue.push({ key: "Due Date", value: dt.toDateString(), url: null,background:false });
                          }
                       
                      }else {
                        itmValue.push({ key: x.Title, value: element[x.Title], url: null,background:false });
                      }
                    }
                  }
                }
